...
function EventCarousel(props) {
  ...
  //驅動程式
  function carouselRun(firstCarouselWidth, carouselIndex, carouselWidth) {
    //index 因扣掉頭要減 1
    let move = -firstCarouselWidth - (carouselIndex - 1) * (carouselWidth + 20);
  ...
  useEffect(() => {
    ...
    //輪播與視窗比例為 1350 / 1900, 輪播單位寬 + 20 margin
    let carouselWidth = ($(".carousel-wrapper").width() * 1350) / 1900 + 20;
  ...
  useEffect(() => {
    ...
    let firstCarouselWidth =
      ...
      ($(".carousel-wrapper").width() - $(".carousel-parts").width()) / 2 +
      20;
    let carouselIndex = $("#carousel-dot-pages li.active").index() + 1;
    carouselRun(firstCarouselWidth, carouselIndex, carouselWidth);
    setLoad(false);
  }, [load]);
